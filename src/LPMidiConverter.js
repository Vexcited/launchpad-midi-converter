(function(scope) {
    ...
    function LPMidiConverter () {
         ...
         */
        this._buildLayout = function (mode) {
            let layout = []
        ...
        this._layouts = {
            programmer: this._buildLayout("programmer"),
            live: this._buildLayout("live")
        };
    };

    // Reference for later
    var lp = new LPMidiConverter();
     ...
     */
    LPMidiConverter.prototype.layout = function (mode) {
        if (this._layouts[mode]) {
            return this._layouts[mode]
        }
        else {
            throw new Error(`Layout "${mode}" doesn't exist or is currently not supported !`);
        }
    }
     ...
     */
    LPMidiConverter.prototype.convert = function (note, from, to) {
        let converted = null;
     ...
     */
    LPMidiConverter.prototype.color = function (velocity) {
    ...
    if (typeof define === "function" && typeof define.amd === "object") {
        ...
        if (!scope.LPMidiConverter) { scope.LPMidiConverter = lp; }
    }
